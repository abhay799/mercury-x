...
def compile_sla(sla,metric_registry,*,version=1,previous_version_id=None):
    ...
    body={"sla":sla.sla_id,"version":version,"requirements":[r.model_dump(mode="json") for r in sla.requirements],
          ...
          "workload_class":sla.workload_class,"metric_registry_fingerprint":metric_registry.fingerprint}
    ...
    return IntelligenceSLO(intelligence_slo_id=sid,source_sla_id=sla.sla_id,version=version,
        ...
        provenance_ids=tuple(sorted(set(prov)|set(sla.provenance_ids))),fingerprint=fp,
        workload_class=sla.workload_class,metric_registry_fingerprint=metric_registry.fingerprint),reasons
