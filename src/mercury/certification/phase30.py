...
import json
from mercury.certification.phase30_checks import CHECKS
...
def _load_manifest(path: Path):
    try:
        ...
        raise ValueError("invalid phase30 certification manifest") from exc
    if set(data) != {"schema_version", "required_gates"}:
        raise ValueError("unsupported manifest fields")
    if data["schema_version"] != "phase30-cert-v1":
...
def evaluate(manifest_path=None):
    path = Path(manifest_path) if manifest_path else Path(__file__).parents[3] / "configs" / "certification" / "phase30.json"
...
def main():
    results = []
    for gate in _load_manifest(Path(__file__).parents[3] / "configs" / "certification" / "phase30.json"):
        ok, evidence = CHECKS[gate]()
        results.append((gate, bool(ok), evidence))
