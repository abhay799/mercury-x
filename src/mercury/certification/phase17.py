...
def evaluate(manifest_path=None):
    p=Path(manifest_path) if manifest_path else Path(__file__).parents[3]/"configs"/"certification"/"phase17.json"
    gates=load_certification_manifest(p,phase=17,known_gates=CHECKS)
    out=[]
    for gate in gates:
