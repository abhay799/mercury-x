...
from mercury.certification.phase18_checks import CHECKS
from mercury.certification.manifest import load_certification_manifest

def evaluate(manifest_path=None):
    p=Path(manifest_path) if manifest_path else Path(__file__).parents[3]/"configs"/"certification"/"phase18.json"
    gates=load_certification_manifest(p,phase=18,known_gates=CHECKS)
    out=[]
    for gate in gates:
