...
from mercury.certification.phase19_checks import CHECKS
from mercury.certification.manifest import load_certification_manifest

def evaluate(manifest_path=None):
    p=Path(manifest_path) if manifest_path else Path(__file__).parents[3]/"configs"/"certification"/"phase19.json"
    gates=load_certification_manifest(p,phase=19,known_gates=CHECKS)
    out=[]
    for gate in gates:
