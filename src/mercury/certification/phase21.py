...
import json
from mercury.certification.phase21_checks import CHECKS

def _load_manifest(path: Path):
    try:
        data = json.loads(path.read_text(encoding="utf-8"))
    except Exception as exc:
        raise ValueError("invalid phase21 certification manifest") from exc
    if set(data) != {"schema_version", "required_gates"}:
        raise ValueError("unsupported manifest fields")
    if data["schema_version"] != "phase21-cert-v1":
...
def evaluate(manifest_path=None):
    path = Path(manifest_path) if manifest_path else Path(__file__).parents[3] / "configs" / "certification" / "phase21.json"
