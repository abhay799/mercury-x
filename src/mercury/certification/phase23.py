...
import json
from mercury.certification.phase23_checks import CHECKS

def evaluate(manifest_path=None):
    path=Path(manifest_path) if manifest_path else Path(__file__).parents[3]/"configs"/"certification"/"phase23.json"
    try:
        data=json.loads(path.read_text(encoding="utf-8"))
    except Exception as exc:
        raise ValueError("invalid phase23 certification manifest") from exc
    if set(data)!={"schema_version","required_gates"}: raise ValueError("unsupported manifest fields")
    if data["schema_version"]!="phase23-cert-v1": raise ValueError("unsupported manifest schema")
