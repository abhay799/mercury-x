from mercury.platform.contracts import PlatformConfig, PlatformMode, PlatformStatus
from mercury.platform.engine import PlatformManager


def test_platform_research_mode_keeps_safe_defaults_and_cpu_local_demo():
    config = PlatformConfig(
        config_id="cfg-1",
        mode=PlatformMode.RESEARCH,
        cpu_local_demo=True,
        allow_cloud=False,
        allow_gpu=False,
        production_safe_defaults=True,
        provenance_ids=("p1",),
    )
    manager = PlatformManager()
    status = manager.validate_config(config)
    assert status is PlatformStatus.OK
    assert manager.demo_mode_enabled(config) is True
